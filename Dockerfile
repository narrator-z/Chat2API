...
CMD ["sh", "-c", "rm -f /tmp/.X*-lock /tmp/.X11-unix/X* 2>/dev/null; Xvfb :99 -screen 0 1024x768x24 & export DISPLAY=:99 && cd /app && export ELECTRON_IS_DEV=0 && export NODE_ENV=production && npm install electron-vite --save-dev && ./node_modules/.bin/electron-vite preview --config /app/electron.vite.config.ts"]
